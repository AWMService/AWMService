...
namespace AWMService.Application.Abstractions.Repositories
{
    public interface IDirectionsRepository
    {
        ...
        Task AddDirectionAsync(string nameKz, string nameRu,string nameEn, string description, int supervisorId, int statusId, int periodId, int actorUserId,
            CancellationToken ct);

        Task UpdateDirectionAsync(int id, string nameKz, string nameRu, string nameEn, string description, int? periodId, int actorUserId, CancellationToken ct);
